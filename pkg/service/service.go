...
func (s *TestingSystemService) SumbitSolution(problemId models.ProblemId) error {
	idToLink := map[models.ProblemId]string{
		uuid.MustParse("7a81f316-add0-4dc9-85a7-28b8fd747f8f"): "1000",
		uuid.MustParse("0960f297-8e9f-492e-90bd-8708772638e3"): "1004",
		uuid.MustParse("e346caa7-5af4-4741-a1a0-d4f5f88a87c2"): "1005",
	}
	id, ok := idToLink[problemId]
	if !ok {
		return errors.NewHTTPError(http.StatusNotFound, "ProblemId is not found in problems map")
	}
	return s.ts.SendSubmission("342187EL", "57", id, "a, b = [int(i) for i in input().split()]\nprint(a + b)")
}
